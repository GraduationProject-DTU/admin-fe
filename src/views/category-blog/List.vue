<template>
    <div>
        <h2 class="intro-y text-lg font-medium mt-4 mb-5">Danh sách danh mục Blog</h2>
        ...
        <div class="grid grid-cols-12 gap-6 mt-5" v-if="!loadingIconAction">
            <div class="intro-y col-span-12 flex flex-wrap xl:flex-nowrap items-center mt-2">
                <button class="btn btn-primary shadow-md mr-2" @click="createBlogCategory">Thêm</button>
            </div>
            <div class="intro-y col-span-12 overflow-auto 2xl:overflow-visible">
                <table class="table table-report -mt-2">
                    ...
                    <tbody>
                        <tr class="intro-x" v-for="(item, index) in listBlogCategory" :key="index" :value="index">
                            <td>{{ index + 1 }}</td>
                            <td>{{ item.title }}</td>
                            <td>{{ item.createdAt.substring(0, 10) }}</td>
                            <td class="table-report__action w-30">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3 text-primary" href="javascript:;" @click="updateBlogCategory(item._id)">
...
export default {
    ...
    methods: {
        async getListBlogCategory() {
            ...
            this.listBlogCategory = res.category
        ...
        async deleteBlogCategory() {
            this.loadingIconAction = true
            const res = await CategoryBlogApi.deleteCategoryBlog(this.idDel)
        ...
        createBlogCategory() {
            this.$router.push({ path: '/category-blog/create' })
        },

        updateBlogCategory(id) {
            this.$router.push({ path: '/category-blog/create', query: { id: id } })
        }
    }
}
</script>
